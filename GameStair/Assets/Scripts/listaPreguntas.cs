...
public class listaPreguntas : MonoBehaviour {
	...
	{
		for (int i=0; i<100; i++) {
			int aux = Random.Range (i, (trivia.Count));
			preg.Add(aux);
		}
		preg.Sort ();

		for (int j=0; j<preg.Count; j++) {
...
/*	
	public bool comprobarOpciones0(List<string>options, int num)
	{
		int cont = 0;
		foreach(string q in options)
		{
			if(saltos(q)==num)
			{
				cont ++;
	...
	void random()
	{
		//i = Random.Range (0, (trivia.Capacity-24));
		//Debug.Log(alea);
		i = 1;
		questionP = trivia [i].getQuestion();
		formato (questionP);

	}
	
	public bool comprobarOpciones2(List<string>options, int num)
	{
		int cont = 0;
		foreach(string q in options)
		{
	...
	public void OnGUI() {
		...
		windowRect = GUI.Window(0, windowRect, DoMyWindow, questionP);
	}*/
/*
	void DoMyWindow(int windowID) {
		...
		if (j == 4) {
			q1 = trivia [i].getOpciones () [0];
			q2 = trivia [i].getOpciones () [1];
			q3 = trivia [i].getOpciones () [2];
			q4 = trivia [i].getOpciones () [3];
			q5 = trivia [i].getOpciones () [4];
		} else {
	...
	{	
		...
		{	
			...
			resp5 = "FC.";
			List<string>opt18 = new List<string>();
			...
			//Pregunta 23
			preg = "Señale cual de las siguientes  moleculas es indispensables para que una proteína sea procesada vía proteosoma:";
			resp1 = "Proteinasa K.";
			resp2 = "Ubiquitina.";
			...
			pregunta p36 = new pregunta(preg, resp1, resp2, resp3, resp4,null, respv,id);
			trivia.Add (p36);
